...
module.exports = {
  ...
  /**
   * checks if item is owned by the requested user
   * @param {String} id - request object
   * @param {Object} model - query object
   * @param {String} currentUserId - Current user's ID
   * @returns {Object} Item
   */
  async checkItemOwnershipAndReturn(id, model, currentUserId) {
    return new Promise((resolve, reject) => {
      model.findById(id, '+owner', (err, item) => {
        console.log(item.owner, currentUserId)
        itemNotFound(err, item, reject, 'NOT_FOUND')
        if (isCurrentUserDocumentOwner(item.owner, currentUserId)) {
